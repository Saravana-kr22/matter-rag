#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""CLI entry point — Test plan coverage gap analysis pipeline.

Loads the cached knowledge graph, identifies spec REQUIREMENT nodes that have no
TEST_CASE coverage edges, and uses the LLM to confirm which gaps are truly uncovered
(vs. implicitly covered by existing tests). Generates an HTML + JSON report.
...
Usage::
    ...
    # Custom config file
    python scripts/run_coverage_analysis.py --config config/config.yaml

Log directory: logs/coverage_analysis_<MMDDYYYY_HHMMSS>/
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Matter RAG — Test plan coverage gap analysis"
    )
    parser.add_argument(
...
def main() -> int:
    ...
    run_dir = configure_pipeline_logging(config, pipeline_name="coverage_analysis")

    from src.engine.run_context import RunContext, set_run_context, _current_run_ctx
    run_ctx = RunContext(
        run_id=run_dir.name,
        run_dir=run_dir,
        client="coverage_analysis",
    )
    ...
    max_llm_calls = args.max_llm_calls if args.max_llm_calls is not None else 0

    from src.engine.graphs.coverage_analysis_graph import build_coverage_analysis_graph
    from src.engine.coverage_analysis_nodes import CoverageAnalysisState

    graph = build_coverage_analysis_graph()
    initial_state: CoverageAnalysisState = {
        "config": config,
        "run_ctx": run_ctx,
        "run_dir": str(run_dir),
